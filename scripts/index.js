...
new Swiper('.goods__block', {
	...
	//Отключает прокрутку на стрелочках "узнать больше" - глюк Swiper.
	preventClicks: true,
	a11y: false,
});
...
//Метод принимает функцию и перебирает
...
myHeaders.append("apikey", "f5FEaSCb363AAZOv1eX2JQW4bTRGfBOW");
...
fetch("https://api.apilayer.com/fixer/latest?symbols=&base=USD", requestOptions)
...
const timer = deadline => {
	...
	const unitSec = document.querySelector('.timer__unit_sec') //секунды доделывала сама
	...
	const descriptionSec = document.querySelector('.timer__unit-description_sec') //секунды доделывала сама
	...
	const getTimeRemaning = () => {
		...
		const seconds = Math.floor((timeRemaining / 1000) % 60); //секунды доделывала сама

		const minutes = Math.floor((timeRemaining / 1000 / 60) % 60);
		const hours = Math.floor((timeRemaining / 1000 / 60 / 60) % 24);
		const days = Math.floor(timeRemaining / 1000 / 60 / 60 / 24);

		return { timeRemaining, seconds, minutes, hours, days}; //секунды доделывала сама
	};

	 const start = () => {
		const timer = getTimeRemaning();
		
		unitSec.textContent = timer.seconds; //секунды доделывала сама
		unitDay.textContent = timer.days;
		unitHour.textContent = timer.hours;
		unitMin.textContent = timer.minutes;

		descriptionSec.textContent = declOfNum(timer.seconds, ['секунда', 'секунды', 'секунд']); //секунды доделывала сама
